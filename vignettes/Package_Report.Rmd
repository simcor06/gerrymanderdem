...
Created by Professor Kyle Walker at the Texas Christian University, this package allows the user to download, store, and load all of the TIGER/Line shapefiles for the available geographic areas of the U.S. Census Bureau. 
...
```
For this package,the census tracs, congressional districts, and the state legislative districts shapefiles were accessed.

Data access:

In this package, the desired geographic data is requested throught the calling of a function.

                                         Tigris Function Example
```{r fig.align='center', out.width= "60%", out.height= "30%"}
include_graphics("fig/Example_functions_Tigris.png")
```

The result of a function is a SpatialPolygonDataFrame. 
                                     
                                    Wisconsin Census Tracts - Top 20
```{r fig.align='center', out.width= "60%", out.height= "30%"}
include_graphics("fig/WI_Census_tracts.png")
```
A benefit of using the Tigris package is the ability to choose between the use of The TIGER/Line shapefiles and the Cartographic Boundary Files. The TIGER/Line are the most detailed datasets in interior areas, and represent the legal boundaries for coastal areas which extend. 
three miles beyond the shorelin a The Cartographic Boundary Files have less detail in interior areas. As the area of the census tracts need to be calculated for this project,the Cartographic Boundary Files provides the most accurate option. To choose the Cartographic Boundary Files, the cb parameter on the tracts function is set to TRUE (Wstate_tracts_NAD83<-tracts(state = 'WI', cb= TRUE))
...
Another package created by Prof. Kyle walker at TCU. In this package the User is able to obtain the tabular data from both decennial US Census and the American Community Survey, through two functions  get_decennial and ger_acs.For this project we used the former. In order to have access to this data, user needs to obtain an API key from the Census  and then use the function census_api_key() to set it in R.

Once api_key is set up, user is able to do queries. The load_variables function of the Tidycensus can help user find the desired variable, by using a filter. The function takes two required arguments: the year of the Census or endyear of the ACS sample, and the dataset - one of "sf1", "sf3", or "acs5" for this project we used "sf1": pop_attempt <- load_variables(2010, dataset = "sf1"), The result of running the load_variables is a data.frame
```{r fig.align='center', out.width= "60%", out.height= "30%"}
...
Once the user has identified the variables to be used, according to the needs of the user two functions are used. 
...
As shown bellow, decennial data is used for this project:
...
```
Using the get_decennial function gives a data.frame as an output
...
The second step will be to obtain a census key from this website: [https://api.census.gov/data/key_signup.html](https://api.census.gov/data/key_signup.html).  This key allows the user to obtain access to the census data provided by the package `tidycensus`, which is a required package for this package. The user will have to load the `tidycenus` package, and then use the function `census_api_key(key, install = TRUE)`.  the key value in this function should be the key provided to you by the census.
