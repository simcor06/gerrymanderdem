...
There are two functions within the tidycensus package, that allow users to obtain the tabular data from both decennial US Census and the American Community Survey. These are the get_decennial and get_acs.For this project we used the former.

The load_variables function of the Tidycensus helps a user find the desired variable. The function takes two required arguments: the year of the Census or endyear of the ACS sample, and the dataset:"sf1"(decennial short form), "sf3" (decennial long form), or "acs5" (ACS).For this project "sf1" is used.
```{r echo = TRUE, eval=FALSE}
